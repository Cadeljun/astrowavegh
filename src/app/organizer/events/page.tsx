...
import { Calendar, MapPin, Zap, ExternalLink, Plus, Eye, Loader2, Trash2, Star, CheckCircle, Award } from 'lucide-react';
...
export default function OrganizerEventsPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'platform_events', deleteTarget.id));
    ...
    <div className="space-y-10 pb-20">
        ...
        {loading ? (
          ...
          <AnimatePresence mode="popLayout">
            {filteredEvents.map((event) => (
              <motion.div key={event.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }}>
                <Card className="h-full group hover:border-gold/30 transition-all flex flex-col relative overflow-hidden" glowColor="muted">
                  <div className="absolute top-0 right-0 w-32 h-32 opacity-5 pointer-events-none">
                     <Zap size={120} className="text-gold absolute -top-8 -right-8" />
                  </div>
                  ...
                  <div className="p-8 space-y-8 flex-1">
                    ...
                    <div className="space-y-2">
                       ...
                       <div className="flex flex-col gap-1 text-[0.7rem] font-bold text-muted uppercase tracking-widest">
                          <span className="flex items-center gap-2.5"><Calendar size={14} className="text-gold" /> {new Date(event.date?.toDate()).toLocaleDateString()} @ {event.startTime}</span>
                    ...
                    <div className="grid grid-cols-2 gap-6 p-4 rounded-xl bg-white/[0.03] border border-white/5">
                       <div className="space-y-1">
                          <p className="text-[0.55rem] label m-0 opacity-40">TALENT TYPE</p>
                  ...
                  <div className="p-8 pt-0 flex gap-4">
                    {event.status === 'open' && (
                      <>
                        <Link href={`/match/${event.id}`} className="flex-1">
                          <Button variant="primary" className="w-full h-12 text-[0.65rem] font-bold shadow-xl">
                            RUN ENGINE <Award size={14} className="ml-2" />
                          </Button>
                        </Link>
                        <button onClick={() => setDeleteTarget(event)} className="p-3 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors border border-red-500/20">
                          <Trash2 size={18} />
                        </button>
                      </>
                    )}
                    ...
                    {event.status === 'booked' && (
                      <Link href="/organizer/bookings" className="flex-1">
                        <Button variant="secondary" className="w-full h-12 text-[0.65rem] font-bold border-white/10">
                          VIEW BOOKING <Eye size={14} className="ml-2" />
                        </Button>
                      </Link>
                    )}
                    ...
                    {event.status === 'completed' && (
                      <Link href="/organizer/bookings" className="flex-1">
                         <Button className="w-full h-12 text-[0.65rem] font-bold bg-purple border-none shadow-[0_0_20px_rgba(168,85,247,0.3)]">
                           {event.rated ? 'VIEW FEEDBACK' : 'RATE PERFORMANCE'} <Star size={14} className="ml-2" />
                         </Button>
                      </Link>
                    )}
                  </div>
          ...
          <div className="col-span-full py-48 text-center space-y-8 opacity-30 border-2 border-dashed border-white/5 rounded-[3rem]">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted uppercase font-bold tracking-widest">You haven't posted any events matching these criteria.</p>
      ...
      {deleteTarget && (
        <ConfirmModal 
          ...
          title="REMOVE EVENT BRIEF?"
          message="This will delete the event and any match history. This action is irreversible."
        />
      )}
    </div>
  );
