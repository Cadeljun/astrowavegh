  ...
  return (
    <div className="flex flex-col w-full">
      <section className="relative h-[50vh] w-full flex items-center justify-center overflow-hidden bg-[var(--color-black)]">
        <div className="absolute inset-0 z-0 opacity-30" style={{ background: 'radial-gradient(ellipse 60% 40% at 50% 50%, rgba(0, 255, 135, 0.1), transparent)' }} />
        <div className="relative z-10 text-center px-6">
          ...
          <motion.h1 variants={fadeUp} initial="hidden" animate="show" className="display-xl text-green text-glow-green mb-4">{hero.heading}</motion.h1>
      ...
      <section className="py-24 px-6 lg:px-12 bg-surface">
        <div className="max-w-screen-2xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="lg:col-span-7">
            <Card className="p-8 md:p-12" glowColor="green">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <input required className="w-full bg-black/40 border border-border-dark rounded-sm p-4 font-body text-white focus:border-green transition-all outline-none" placeholder="Full Name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                  <input required type="email" className="w-full bg-black/40 border border-border-dark rounded-sm p-4 font-body text-white focus:border-green transition-all outline-none" placeholder="Email Address" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                </div>
                <select className="w-full bg-black/60 border border-border-dark rounded-sm p-4 font-body text-white focus:border-green transition-all outline-none" value={formData.subject} onChange={e => setFormData({...formData, subject: e.target.value})}><option value="Event Booking">Event Booking</option><option value="Talent Inquiry">Talent Inquiry</option><option value="General Enquiry">General Enquiry</option></select>
                <textarea required rows={5} className="w-full bg-black/40 border border-border-dark rounded-sm p-4 font-body text-white focus:border-green transition-all outline-none resize-none" placeholder="Message..." value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} />
